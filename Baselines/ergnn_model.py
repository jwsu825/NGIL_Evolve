...
class NET(torch.nn.Module):
    ...
    def observe_task_IL_batch(self, args, g, dataloader, features, labels, t, train_ids, ids_per_cls, dataset):
        ...
        #jwsu:compute and store old rep
        # if t != 0:
        #     for oldt in range(t):
        ...
        for input_nodes, output_nodes, blocks in dataloader:
            ...
            if t != 0:
                for oldt in range(t):
                    ...
                    # print('I am here')
                    # input('spotting error')
                    
                    # jwsu
                    # old_task_rep = self.net.second_last_h
                    # alpha1 = 0
                    # alpha2 = 0
                    # # loss =  loss + alpha1 * MMD(new_task_rep, old_task_rep) + alpha2 * MMD(old_task_rep,torch.tensor(self.rep_oldt_old[oldt]))
                    # loss =  loss + alpha1 * MMD(new_task_rep, old_task_rep) 
    ...
    def observe_task_IL(self, args, g, features, labels, t, train_ids, ids_per_cls, dataset):
        ...
        if t!=0:
            for oldt in range(t):
                ...
                alpha1 = 0.99
                alpha2 = 0.99
                loss = alpha1 * loss + (1-alpha1) * MMD(new_task_rep, old_task_rep)
